"use strict";
exports.__esModule = true;
var admin = require("firebase-admin");
var database_service_1 = require("./import/database-service");
var import_service_1 = require("./import/import-service");
var params = {
    type: "service_account",
    project_id: "test-cb955",
    private_key_id: "8e988b31cfe489a63f5d953eadf3c5399ecd04c3",
    private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    client_email: "firebase-adminsdk-ycai2@test-cb955.iam.gserviceaccount.com",
    client_id: "118409361764057885315",
    auth_uri: "https://accounts.google.com/o/oauth2/auth",
    token_uri: "https://oauth2.googleapis.com/token",
    auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
    client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-ycai2%40test-cb955.iam.gserviceaccount.com"
};
admin.initializeApp({
    credential: admin.credential.cert(params)
});
var databaseService = new database_service_1.DatabaseService();
var importService = new import_service_1.ImportService(databaseService);
var jsonData = "{\"config/\":  {\"default\": {\"bundlesVersions\": {\"translations_1\": 0},\"bundles/\": {\"translations_1\": {\"alkjlkj\": [0, 1, 2]}}}}, \"config2/\": {\"moje\": {\"xd\": [1,2]}}}";
var json = JSON.parse(jsonData);
importService.parseJson(json, "");
